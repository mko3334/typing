...
code = code.replace(
  /<\/main>\s*\{appScreen === 'playing' && \(/, // Avoid duplicating if already there
  `</main>`
);
...
code = code.replace(
  /<button\s+onClick=\{handleRestart\}\s+className="premium-button bg-sky-500 hover:bg-sky-600 text-white text-xl px-6 py-3 flex items-center gap-3 mt-4"\s*>\s*<RefreshCcw className="w-6 h-6" \/>\s*もういちどあそぶ\s*<\/button>/,
...
code = code.replace(
  /<button onClick=\{handleRestart\} className="mt-4 text-gray-400 hover:text-gray-600 font-bold underline text-sm">\s*やっぱりタイピングにもどる\s*<\/button>/,
